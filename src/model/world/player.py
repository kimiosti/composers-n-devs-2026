...
class Player(Entity):
    ...
    def __init__(self, pos: "Tuple[float, float]", size: "Tuple[float, float]",
        ...
        """Instantiates a player-controlled game entity.
        
        Positional arguments:  
         - `pos`: the coordinates of the top-left corner of the player's hitbox \
            when spawned.  
         - `size`: the dimension of the player's hitbox.  
         - `name`: the player's name, useful for retrieving the proper texture.  
         - `direction`: the direction towards which the player is facing when spawned."""
        super().__init__(pos, size, name, direction)
    ...
    def move(self, direction: "Direction", dt: "int") -> "Rect":
        ...
        Note that the game world coordinate system is left-handed. The origin is the top-left corner \
            of the map, with x increasing towards the right and y increasing towards the bottom."""
        self._direction = direction
        return self._hitbox.move(
            PLAYER_SPEED * dt if direction == Direction.RIGHT else
            - PLAYER_SPEED * dt if direction == Direction.LEFT else 0,
            PLAYER_SPEED * dt if direction == Direction.DOWN else
            - PLAYER_SPEED * dt if direction == Direction.UP else 0
        )
